...
import com.mikky.corebanking.authenticationservice.application.command.dto.SignupResponse;
import com.mikky.corebanking.authenticationservice.application.command.dto.ValidationResponse;
import com.mikky.corebanking.authenticationservice.application.command.service.AuthCommandService;
import com.mikky.corebanking.authenticationservice.application.command.service.TokenBlacklistCommandService;
...
import com.mikky.corebanking.authenticationservice.infrastructure.security.CustomUserDetailsService;
...
public class AuthController {
    ...
    public ResponseEntity<String> signOutUser(@RequestHeader("Authorization") String authHeader) {
        try {
            tokenBlacklistCommandService.blackListToken(authHeader);
            return ResponseEntity
    ...
    public ResponseEntity<ValidationResponse> validateToken(@RequestHeader("Authorization") String authHeader) {
        try {
            authQueryService.validateToken(authHeader);
            return ResponseEntity
                    .status(HttpStatus.OK)
                    .body(new ValidationResponse("Valid Token!"));
        } catch (Exception e) {
            return ResponseEntity
                    .status(HttpStatus.BAD_REQUEST)
                    .body(new ValidationResponse(e.getMessage()));
        }
    }
